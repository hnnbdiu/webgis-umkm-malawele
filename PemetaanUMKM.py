...
/* Bungkus komponen GPS jadi TOMBOL BULAT KECIL, dan ikonnya di-ZOOM
   (transform: scale) supaya memenuhi penuh kotak wrapper — tidak ada
   lagi sisa border/area kosong di sekelilingnya. */
...
/* Iframe dipatok ke ukuran asli tombolnya (44x44), lalu di-scale 1.35x
   dan ditempatkan pas di tengah wrapper (position:absolute + inset:0
   dengan transform) supaya ikon membesar mengisi penuh kotak 48x48.

   CATATAN TUNING — kalau setelah redeploy hasilnya masih kurang pas:
   - Kalau MASIH ada sisa garis/area kosong di tepi → naikkan angka scale()
     sedikit (mis. 1.35 -> 1.5).
   - Kalau ikon jadi KEPOTONG/terlalu besar → turunkan angka scale()
     (mis. 1.35 -> 1.2). */
