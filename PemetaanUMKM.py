...
def load_data():
    try:
        ...
        if len(df) >= 3:
            # 1. Eksekusi K-Means awal
            kmeans = KMeans(n_clusters=3, random_state=42, n_init=10)
            klaster_mentah = kmeans.fit_predict(df[['lat', 'lon']])
            df['cluster_mentah'] = klaster_mentah
            
            # 2. Hitung jumlah anggota tiap klaster (Kepadatan)
            kepadatan_klaster = df['cluster_mentah'].value_counts()
            
            # 3. Ekstrak indeks klaster yang sudah terurut dari yang Paling Padat ke Paling Renggang
            urutan_kepadatan = kepadatan_klaster.index.tolist()
            
            # 4. Buat kamus pemetaan ulang (Mapping)
            # Indeks 0 (Terpadat) akan selalu dipaksa menjadi ID 0 (Merah pada Folium)
            # Indeks 1 (Menengah) akan selalu dipaksa menjadi ID 1 (Biru pada Folium)
            # Indeks 2 (Terenggang) akan selalu dipaksa menjadi ID 2 (Hijau pada Folium)
            mapping_cerdas = {
                urutan_kepadatan[0]: 0,
                urutan_kepadatan[1]: 1,
                urutan_kepadatan[2]: 2
            }
            
            # 5. Terapkan pemetaan ke dataframe utama dan hapus kolom mentah
            df['cluster'] = df['cluster_mentah'].map(mapping_cerdas)
            df = df.drop(columns=['cluster_mentah'])
