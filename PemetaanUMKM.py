...
/* Bungkus komponen GPS jadi TOMBOL BULAT KECIL saja (bukan strip lebar).
   Pendekatan sebelumnya (invert + blend) tidak bisa diandalkan karena warna
   asli komponen bukan hitam/putih murni. Solusi yang pasti berhasil: WRAPPER
   dipotong (overflow:hidden) sekecil ukuran tombolnya saja, sehingga sisa
   area kosong komponen (yang tidak bisa kita warnai dari luar iframe)
   tidak pernah ikut terlihat sama sekali. */
...
/* Iframe tetap dirender selebar aslinya (supaya tombolnya tidak gepeng/rusak),
   tapi karena wrapper di atas overflow:hidden & lebih kecil, bagian kosong
   di sisi kanan iframe otomatis terpotong dan tidak pernah tampak. */
...
if menu_pilihan == "📍 Peta UMKM (Utama)":
    ...
    with col_q:
        query_user = st.text_input("🔍  Konsol Pencarian", placeholder="cth: sa mau pi makan di warung dolo")
    with col_k:
